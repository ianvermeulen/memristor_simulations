...
Input,S,Output = init_network(N, M)
...
def graph_synapses():
  """
  Graph all the synapses using colormaps
  """
  transposed_weights = np.reshape(S.w, (N, M)).T
  for i in range(0, M):
    if (i%6 == 0):
      figure(i/6 + 1, figsize=(14,9))
    subplot(2,3,(i%6)+1)
...
try:
  ...
  for i in range(start_round, TRAINING_ROUNDS):
    ...
    while (j < TRAINING_SIZE):
      ...
      total_spikes = float(spike_mon.num_spikes)
      spike_train = spike_mon.spike_trains()
      for k in range(0,M):
        # print 'total spikes: %d' % spike_mon.num_spikes
        print 'total spikes for neuron %d: %d' % (k, len(spike_train[k]))
        # print 'neuron %d, activity:' % i
        Output.v_th[k]+= GAMMA*(len(spike_train[k])/total_spikes-ACTIVITY_TARGET)
        Output.v_th[k] = max(Output.v_th[k], MIN_VTH)
        # print 'change: %f, value: %f' % (GAMMA*(len(spike_train[k])/total_spikes-ACTIVITY_TARGET), Output.v_th[k])
